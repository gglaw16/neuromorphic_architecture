...
class CF(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        self.input_layer = nn.Conv2d(1,32,kernel_size=5)
        
        self.hidden_layer = nn.Linear(in_features=24*24*32, 
    ...
    def forward(self, images):
        ...
        features = torch.relu(self.layers[1](features.view(images.shape[0], -1)))
        self.activations.append(features)
...
class CF_spikes(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        self.input_layer = spike.Conv2d(1,32,kernel_size=5)
        
        self.hidden_layer = spike.Linear(in_features=24*24*32, 
                                      out_features=HIDDEN_NEURONS)
        
        self.output_layer = spike.Linear(in_features=HIDDEN_NEURONS, 
                                                out_features=kwargs["output_shape"])
    ...
    def _save_memory(self, in_features):
        ...
        features = torch.relu(self.hidden_layer.save_memory(features.view(features.shape[0], -1)))
...
def test_mnist_classifier(show=True):
    ...
    if os.path.isfile('test_data/mnist_classifier_conv.pth'):
        model.load_state_dict(torch.load('test_data/mnist_classifier_conv.pth',
        ...
        # save the model
        torch.save(model.state_dict(), 'test_data/mnist_classifier_conv.pth')
